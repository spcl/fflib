...
#ifdef USE_FFLIB
#define MPI_Request ff_schedule_h
#endif
...
static PetscErrorCode MPIPetsc_Iallreduce(void *sendbuf,void *recvbuf,PetscMPIInt count,MPI_Datatype datatype,MPI_Op op,MPI_Comm comm,MPI_Request *request)
{
  ...
  //printf("allreduce of %i elements\n", count);
  PetscFunctionBegin;
...
#if defined(USE_FFLIB)
  if (op!=MPI_SUM) { printf("Different operator than MPI_SUM used. Aborting.\n"); exit(-1); }
  if (datatype!=MPI_DOUBLE) { printf("Different datatype than MPI_DOUBLE used. Aborting.\n"); exit(-1); }
  *request = ff_allreduce(sendbuf, recvbuf, count, 0, FF_SUM, FF_DOUBLE);
  ff_schedule_post(*request, 1);
  //printf("comb.c: allocated sched: %i\n", *request);
#elif defined(PETSC_HAVE_MPI_IALLREDUCE)
  //printf("op: %x: MPI_MIN: %x; MPI_MAX: %x; MPI_SUM: %x\n", op, MPI_MIN, MPI_MAX, MPI_SUM);
  ...
  if (sr->async) {              /* Bad reuse, setup code copied from PetscSplitReductionApply(). */
    ...
    if (size == 1) {
      ...
      if (sum_flg + max_flg + min_flg > 1) {
        ...
        printf("strange reduce executed here: %x\n", PetscSplitReduction_Op);
        ierr = MPIPetsc_Iallreduce(lvalues,gvalues,2*numops,MPIU_SCALAR,PetscSplitReduction_Op,comm,&_req);CHKERRQ(ierr);
      } else if (max_flg) {   /* Compute max of real and imag parts separately, presumably only the real part is used */
        //printf("iallreduce with op: MPIU_MAX %x\n", MPIU_MAX);
        ierr = MPIPetsc_Iallreduce((PetscReal*)lvalues,(PetscReal*)gvalues,cmul*numops,MPIU_REAL,MPIU_MAX,comm,&_req);CHKERRQ(ierr);
      } else if (min_flg) {
        //printf("iallreduce with op: MPIU_MIN %x\n", MPIU_MIN);
        ierr = MPIPetsc_Iallreduce((PetscReal*)lvalues,(PetscReal*)gvalues,cmul*numops,MPIU_REAL,MPIU_MIN,comm,&_req);CHKERRQ(ierr);
      } else {
        //printf("iallreduce with op: MPIU_SUM %x\n", MPIU_SUM);
        ierr = MPIPetsc_Iallreduce(lvalues,gvalues,numops,MPIU_SCALAR,MPIU_SUM,comm,&_req);CHKERRQ(ierr);
      }
...
#if defined(USE_FFLIB)
    //printf("comb.c: waiting for sched: %i\n", sr->sched);
    ff_schedule_wait(sr->sched);
    ff_schedule_free(sr->sched);
#else
