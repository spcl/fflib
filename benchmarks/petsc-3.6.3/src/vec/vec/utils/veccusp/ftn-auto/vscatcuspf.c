...
extern "C" {
#endif
PETSC_EXTERN void PETSC_STDCALL  vecscatterinitializeforgpu_(VecScatter inctx,Vec x,ScatterMode *mode, int *__ierr ){
*__ierr = VecScatterInitializeForGPU(
	(VecScatter)PetscToPointer((inctx) ),
	(Vec)PetscToPointer((x) ),*mode);
}
PETSC_EXTERN void PETSC_STDCALL  vecscatterfinalizeforgpu_(VecScatter inctx, int *__ierr ){
