...
extern "C" {
#endif
PETSC_EXTERN void PETSC_STDCALL  kspnashsetradius_(KSP ksp,PetscReal *radius, int *__ierr ){
*__ierr = KSPNASHSetRadius(
	(KSP)PetscToPointer((ksp) ),*radius);
}
PETSC_EXTERN void PETSC_STDCALL  kspnashgetnormd_(KSP ksp,PetscReal *norm_d, int *__ierr ){
*__ierr = KSPNASHGetNormD(
	(KSP)PetscToPointer((ksp) ),norm_d);
}
PETSC_EXTERN void PETSC_STDCALL  kspnashgetobjfcn_(KSP ksp,PetscReal *o_fcn, int *__ierr ){
