...
extern "C" {
#endif
PETSC_EXTERN void PETSC_STDCALL  kspstcgsetradius_(KSP ksp,PetscReal *radius, int *__ierr ){
*__ierr = KSPSTCGSetRadius(
	(KSP)PetscToPointer((ksp) ),*radius);
}
PETSC_EXTERN void PETSC_STDCALL  kspstcggetnormd_(KSP ksp,PetscReal *norm_d, int *__ierr ){
*__ierr = KSPSTCGGetNormD(
	(KSP)PetscToPointer((ksp) ),norm_d);
}
PETSC_EXTERN void PETSC_STDCALL  kspstcggetobjfcn_(KSP ksp,PetscReal *o_fcn, int *__ierr ){
