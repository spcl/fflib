...
extern "C" {
#endif
PETSC_EXTERN void PETSC_STDCALL  matcreatesubmatrix_(Mat A,IS isrow,IS iscol,Mat *newmat, int *__ierr ){
*__ierr = MatCreateSubMatrix(
	(Mat)PetscToPointer((A) ),
	(IS)PetscToPointer((isrow) ),
	(IS)PetscToPointer((iscol) ),newmat);
}
PETSC_EXTERN void PETSC_STDCALL  matsubmatrixupdate_(Mat N,Mat A,IS isrow,IS iscol, int *__ierr ){
