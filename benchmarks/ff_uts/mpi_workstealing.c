...
/** Asynch stuff **/
static int adegree = 10;
static ff_sched_info cinfo;
static ff_container_h colls;
static uint64_t tosteal;
static uint64_t localWorkAndRank;
...
void updateAsynchBuff(StealStack *s){
	//int c = s->localWork;
	//int csize = ff_get_size();
	uint32_t rank = (uint32_t) ff_get_rank();
	uint64_t c = ((uint32_t) s->localWork);
	c = c << 32 | rank;
	localWorkAndRank = c; 
}
...
void ws_make_progress(StealStack *s)
{
	...
	//re-fill the container if needed
	
 //   int k a= 0;
 //   if (k%3==0 || 1==1){
        int flushed = ff_container_flush(colls);
	    for (int i=0; i<flushed; i++) ff_container_increase_async(colls);
  //  }
  //  k++;
	return;
}
...
static int ccount=0;
ff_schedule_h create_asynch_allreduce(ff_sched_info info){
	void * sndbuff = (void *) &localWorkAndRank;
	void * rcvbuff = (void *) &tosteal;

	return ff_oneside_allreduce(sndbuff, rcvbuff, info.count, info.unitsize, info.tag + ccount, info._operator, info.datatype);
}
...
int ensureLocalWork(StealStack *s)
{
	...
	while (s->localWork == 0) {
		...
		if (wrout_request == MPI_REQUEST_NULL && my_color != PINK) {
			...
			/* calc last_steal with asynch. allreduce */
			//last_steal = (last_steal + 1) % comm_size;
			//if (last_steal == comm_rank) last_steal = (last_steal + 1) % comm_size;
			//buffer: s->local_work
		    
            printf ("rank %i is starting stealing\n", ff_get_rank());
            sched = ff_container_get_next(colls);
			if (sched==FF_EMPTY) printf("CONTAINER EMPTY\n");
			ff_schedule_satisfy_user_dep(sched);
			ff_schedule_wait(sched);
			last_steal = tosteal & 0x00001111;
            //printf("aa\n");a
            printf("rank %i is staling from %i\n", ff_get_rank(), last_steal); 
            int k = 0;
            int flushed = 0;
            /*if (k%3==0 || 1==1){
                flushed = ff_container_flush(colls);
	            for (int i=0; i<flushed; i++) ff_container_increase_async(colls);
            }
            k++;*/
            //if (flushed>0) printf("rank %i flushed %i schedule\n", ff_get_rank(), flushed);
...
StealStack* ss_init(int *argc, char ***argv)
{
	...
	//Setting up info for collectives
	cinfo.count=1;
	cinfo._operator=FF_MAX;
	cinfo.datatype=FF_UINT64_T;
	cinfo.userdata = NULL;
	cinfo.unitsize = sizeof(uint64_t);
	cinfo.tag=1;

	colls = ff_container_create(cinfo, create_asynch_allreduce);
	if (colls==FF_FAIL) printf("Error while creating container\n");
	for (int i=0; i<adegree; i++) ff_container_increase_async(colls);
	if (ff_container_start(colls)!=FF_SUCCESS) printf("Error while starting the container\n");
