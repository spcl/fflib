...
FILE       *dotfile;
const char *filename = "uts_tree.dot";
...
Result parTreeSearch(int depth, Node *parent, int node_num) {
...
#pragma mta loop future
    for (i = 0; i < numChildren; i++) {
      ...
      fprintf(dotfile, "  %d -> %d;\n", node_num, node_num + r.size);
      Result c = parTreeSearch(depth+1, &child, node_num + r.size);
#ifdef __MTA__
...
int main(int argc, char *argv[]) {
  ...
  if (!dotfile) {
    perror("Couldn't open dot file");
    exit(1);
  }

  fprintf(dotfile, "digraph UTS { \n");
...
#pragma mta fence
  ...
  Result r = parTreeSearch(0, &root, 0);
